...
@api_view(['GET'])
@permission_classes([AllowAny])
def get_user_branch(request, user_id):
    """Get branch_id for a specific user"""
    try:
        user = User.objects.get(id=user_id)
        
        # Assuming you have a relationship between User and Branch
        # This could be through a Profile model or direct foreign key
        # You'll need to adjust this based on your actual model structure
        
        # Option 1: If User has a direct branch field
        # branch = user.branch
        
        # Option 2: If you have a Profile model linking User to Branch
        # branch = user.profile.branch
        
        # Option 3: If Branch has a foreign key to User
        branch = Branch.objects.filter(user=user).first()
        
        if branch:
            return Response({
                'branch_id': branch.id,
                'branch_name': branch.name if hasattr(branch, 'name') else None
            })
        else:
            return Response({
        ...
        logger.error(f"Error fetching branch for user {user_id}: {str(e)}")
        return Response({
